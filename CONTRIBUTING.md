...
Some HTTP adapters are generated by separately versioned external integrations.
Admit their exact content-addressed candidates without editing them, then run
`bin/regenerate` to rebuild OpenAPI, the TypeScript client, and the served bundle
in order. These paths are marked unmergeable in `.gitattributes`; resolve a
conflict by regenerating the owning stage. See `docs/PARALLEL_WORK.md`.
