...
use fleetd::{
    ...
    ToolBudget, TurnPolicy, TurnSource, harness_acp_capabilities,
};
use serde_json::json;
use uuid::Uuid;
...
fn parse_arguments() -> Result<Arguments, Box<dyn std::error::Error>> {
    ...
    Ok(Arguments {
        plugin: required(&mut arguments, "OpenCode plugin executable")?,
        opencode: required(&mut arguments, "OpenCode executable")?,
        expected_version: required(&mut arguments, "expected OpenCode version")?,
        model: required(&mut arguments, "OpenCode provider/model")?,
        home: required(&mut arguments, "OpenCode home directory")?,
...
fn plugin_spec(arguments: &Arguments) -> PluginSpec {
    PluginSpec::new("fleetd.harness.opencode", PathBuf::from(&arguments.plugin))
        .with_config(json!({
            "executable": arguments.opencode,
...
async fn run_prompt(
    ...
    harness
        .start_turn(&StartTurn {
            ...
            session_ref,
            source: TurnSource {
                agent_id: "opencode-qualification".to_owned(),
                message_id: Uuid::new_v4().to_string(),
                channel_id: "opencode-qualification".to_owned(),
