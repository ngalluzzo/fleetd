...
fn fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/mock_codex.py")
}

fn plugin_spec() -> PluginSpec {
    PluginSpec::new(
        "fleetd.harness.codex",
        env!("CARGO_BIN_EXE_fleetd-harness-codex"),
    )
    .with_config(json!({
        "executable": fixture_path(),
        "expected_version": "1.6.2",
        "home": std::env::temp_dir(),
        "codex_home": std::env::current_dir().expect("current directory"),
        "path": "/usr/bin:/bin",
        "term": "xterm-256color",
...
#[tokio::test]
async fn codex_is_a_distinct_typed_harness_plugin() {
    let process = PluginProcess::start(plugin_spec())
        .await
        .expect("start Codex plugin");
    assert_eq!(process.manifest().plugin.id, "fleetd.harness.codex");
    let mut harness = process.into_harness_acp().expect("typed harness");
    let description = harness.describe().await.expect("describe");
    assert_eq!(description.runtime.name, "Codex");
    assert_eq!(description.runtime.version, "1.6.2");
    ...
    harness.shutdown().await.expect("shutdown Codex plugin");
}
